		...
		case 16 :
				  {
					...
					while(*s!=NULL)
					{
						while (x<640-flag && (*s!=NULL))
						{
							...
							quma=s[0]-0xa0;                      //�������?
							weima=s[1]-0xa0;                     //���λ��?
							offset=(94*(quma-1)+(weima-1))*32L;   //����?��ʾ�ĺ������ֿ��ļ��е�ƫ��
		...
		case 48:
				  {
					...
					while(*s!=NULL)
					{
						while (x<640-flag && (*s!=NULL))
						{
							...
							quma=s[0]-0xa0;                      //�������?
							weima=s[1]-0xa0;                     //���λ��?
							offset=(94*(quma-1)+(weima-1))*288L;   //����?��ʾ�ĺ������ֿ��ļ��е�ƫ��
							fseek(hzk_p,offset,SEEK_SET);         //�ض�λ�ļ�ָ��
...
void put_all(int x, int y, char* s, int len, int color)//x,y表示坝标�?*s表示覝输入字符串，len表示一行的字的�?�?,color表示颜色
{
	char c[2];//英文
	char z[3];//�?�?
	int i = 0;
	...
	for (i = 0; i < m; i++)
	{
		if (s[i] >= 32 && s[i] <= 126)
		{
			...
			c[1] = '\0';
			j=n%len;
			outtextxy(x + 14 * j + 6, y + 9 + 20 * n1,c);
			n++;
		}
		else
		{
